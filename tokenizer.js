export function tokenizeWord(word) {
    ...
    if (prefix) {
        ...
        if (prefixToken in nobreak &&
            nobreak[prefixToken].includes(stem(word))) {
            return [word];
        }
        return [
            prefixToken,
...
function stem(token) {
    ...
    else if ((suf = endsinArr(r1txt, ["euse", "euses", "euthe", "euthes"])) !== "") {
    ...
    if (step2aDone && token === beforeStep2a) {
        ...
        else if ((suf = endsinArr(rvtxt, ["ions"])) !== "" &&
